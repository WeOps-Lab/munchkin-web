...
const AnnotationModal: React.FC<AnnotationModalProps> = ({ visible, showMarkOnly, annotation, onSave, onCancel, onRemove }) => {
  ...
  const handleSave = async () => {
    const { question, answer } = annotation;
    setSaving(true);
    const payload = {
      question: questionContent,
      knowledge_base_id: parseInt(knowledgeBase.toString()),
      answer_id: showMarkOnly ? answer.id : null,
      content: answerContent,
      tag_id: annotation?.tagId || 0
    };

    try {
      const data = await post('/bot_mgmt/history/set_tag', payload);
      message.success(t('chat.annotationSaved'));
      onSave({
        answer: { ...answer, content: answerContent },
  ...
  const handleRemoveAnnotation = async () => {
    setSaving(true);
    try {
      await post('/bot_mgmt/history/remove_tag/', {
        tag_id: annotation.tagId
      });
